...
const makeResult = (overrides: Partial<ScreenerResult> = {}): ScreenerResult => ({
  symbol: "AAPL",
  name: "Apple Inc.",
  sector: "Technology",
  price: 150,
...
describe("ScreenerService", () => {
  describe("matchesFilter", () => {
    it("gt: returns true when field > value", () => {
      ...
      const filter: ScreenerFilter = { field: "peRatio", operator: "gt", value: 25, label: "P/E > 25" };
    ...
    it("lt: returns true when field < value", () => {
      ...
      const filter: ScreenerFilter = { field: "peRatio", operator: "lt", value: 15, label: "P/E < 15" };
      expect(screenerService.matchesFilter(result, filter)).toBe(true);
    });
    ...
    it("eq: returns true when field == value (number)", () => {
      ...
      const filter: ScreenerFilter = { field: "peRatio", operator: "eq", value: 28, label: "P/E = 28" };
      expect(screenerService.matchesFilter(result, filter)).toBe(true);
    });

    it("eq: returns true when field == value (string)", () => {
      const result = makeResult({ sector: "Technology" });
      const filter: ScreenerFilter = { field: "sector", operator: "eq", value: "Technology", label: "Sector = Tech" };
      expect(screenerService.matchesFilter(result, filter)).toBe(true);
    });
    ...
    it("gte: returns true when field >= value", () => {
      ...
      const filter: ScreenerFilter = { field: "volume", operator: "gte", value: 1_000_000, label: "Vol >= 1M" };
      expect(screenerService.matchesFilter(result, filter)).toBe(true);
    });
    ...
    it("lte: returns true when field <= value", () => {
      ...
      const filter: ScreenerFilter = { field: "price", operator: "lte", value: 100, label: "Price <= 100" };
      expect(screenerService.matchesFilter(result, filter)).toBe(true);
    });
    ...
    it("between: returns true when field is within range (inclusive)", () => {
      ...
      const filter: ScreenerFilter = { field: "peRatio", operator: "between", value: [15, 25], label: "P/E 15-25" };
      expect(screenerService.matchesFilter(result, filter)).toBe(true);
    });
    ...
    it("returns false for undefined/missing fields", () => {
      ...
      const filter: ScreenerFilter = { field: "peRatio", operator: "gt", value: 10, label: "P/E > 10" };
      expect(screenerService.matchesFilter(result, filter)).toBe(false);
    });

    it("returns false for non-existent fields", () => {
      const result = makeResult();
      const filter: ScreenerFilter = { field: "nonExistentField", operator: "gt", value: 10, label: "test" };
      expect(screenerService.matchesFilter(result, filter)).toBe(false);
    });

    it("returns false for invalid operator", () => {
      const result = makeResult({ peRatio: 20 });
      const filter = { field: "peRatio", operator: "invalid" as any, value: 10, label: "test" };
      expect(screenerService.matchesFilter(result, filter)).toBe(false);
    });

    it("between: returns false when value is not a 2-element array", () => {
      const result = makeResult({ peRatio: 20 });
      const filter: ScreenerFilter = { field: "peRatio", operator: "between", value: 10 as any, label: "test" };
      expect(screenerService.matchesFilter(result, filter)).toBe(false);
    });

    it("in: returns false when value is not an array", () => {
      const result = makeResult({ sector: "Technology" });
      const filter: ScreenerFilter = { field: "sector", operator: "in", value: "Technology" as any, label: "test" };
      expect(screenerService.matchesFilter(result, filter)).toBe(false);
    });
  });
  ...
  describe("matchesAllFilters", () => {
    it("returns true when result matches all filters (AND logic)", () => {
      const result = makeResult({ peRatio: 20, sector: "Technology", volume: 2_000_000 });
    ...
    it("returns false when result fails any single filter", () => {
      const result = makeResult({ peRatio: 30, sector: "Technology", volume: 2_000_000 });
      const filters: ScreenerFilter[] = [
        { field: "peRatio", operator: "lt", value: 25, label: "P/E < 25" },
        { field: "sector", operator: "eq", value: "Technology", label: "Sector = Tech" },
      ];
  ...
  describe("filterResults", () => {
    const results: ScreenerResult[] = [
      makeResult({ symbol: "AAPL", peRatio: 28, sector: "Technology", volume: 5_000_000 }),
      makeResult({ symbol: "MSFT", peRatio: 35, sector: "Technology", volume: 3_000_000 }),
      makeResult({ symbol: "JNJ", peRatio: 15, sector: "Healthcare", volume: 1_000_000 }),
      makeResult({ symbol: "XOM", peRatio: 10, sector: "Energy", volume: 800_000 }),
    ];
  ...
  describe("calculateValuationContext", () => {
    it("returns 'overpriced' when P/E > 30", () => {
      ...
      expect(screenerService.calculateValuationContext(result)).toBe("overpriced");
    });
    ...
    it("returns 'underpriced' when P/E < 15 AND P/B < 1.5 AND PEG < 1", () => {
      ...
      expect(screenerService.calculateValuationContext(result)).toBe("underpriced");
    });
    ...
    it("returns 'fair' when all metrics are undefined", () => {
      const result = makeResult({ peRatio: undefined, pbRatio: undefined, pegRatio: undefined });
      expect(screenerService.calculateValuationContext(result)).toBe("fair");
    });

    it("returns 'fair' when only some metrics present and none trigger overpriced", () => {
      const result = makeResult({ peRatio: 20, pbRatio: undefined, pegRatio: undefined });
      expect(screenerService.calculateValuationContext(result)).toBe("fair");
    });

    it("does not return 'underpriced' when any metric is missing", () => {
      const result = makeResult({ peRatio: 10, pbRatio: 1.0, pegRatio: undefined });
      expect(screenerService.calculateValuationContext(result)).not.toBe("underpriced");
    });
    ...
    it("returns 'overpriced' at boundary: P/E exactly 30 is fair, P/E 30.01 is overpriced", () => {
      ...
      expect(screenerService.calculateValuationContext(over)).toBe("overpriced");
    });
  });
  ...
  describe("fetchScreenerData", () => {
    it("returns filtered results with valuation context", async () => {
      ...
      vi.spyOn(marketDataMod.marketDataService, "getStockPerformance").mockResolvedValue([
        { symbol: "AAPL", name: "Apple", price: 150, changePercent: 2.5, sector: "Technology", marketCap: 2_500_000_000_000 },
        { symbol: "XOM", name: "Exxon", price: 100, changePercent: -1.2, sector: "Energy", marketCap: 400_000_000_000 },
      ]);

      const results = await screenerService.fetchScreenerData([]);
      expect(results).toHaveLength(2);
      results.forEach((r) => {
        expect(["overpriced", "underpriced", "fair"]).toContain(r.valuationContext);
      });
